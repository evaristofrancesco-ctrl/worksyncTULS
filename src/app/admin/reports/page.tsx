...
export default function ReportsPage() {
  ...
  const reportData = useMemo(() => {
    ...
    return employees.map(emp => {
      // Calcolo ore lavorate effettive
      const empEntries = allEntries.filter(entry => {
        ...
        return checkIn && isWithinInterval(checkIn, { start: monthStart, end: monthEnd });
      });

      let totalWorkHours = 0;
      empEntries.forEach(entry => {
        if (entry.checkInTime && entry.checkOutTime) {
          const diff = parseISO(entry.checkOutTime).getTime() - parseISO(entry.checkInTime).getTime();
          totalWorkHours += diff / (1000 * 60 * 60);
        }
      });

      // Calcolo ore richieste (Ferie, Malattia, Permessi)
      const empRequests = allRequests.filter(req => {
        if (req.employeeId !== emp.id || (req.status !== "Approvato" && req.status !== "APPROVED")) return false;
      ...
      return {
        ...
        vacationHours: (vacationDays * 8).toFixed(1),
        sickHours: (sickDays * 8).toFixed(1),
        permitHours: permitHours.toFixed(1),
        totalHours: (totalWorkHours + vacationDays * 8 + sickDays * 8 + permitHours).toFixed(1)
      }
    });
        ...
        <CardHeader className="border-b bg-slate-50/50">
          <div className="flex items-center justify-between">
            <CardTitle className="text-sm font-black uppercase tracking-wider text-slate-500 flex items-center gap-2">
              <Users className="h-4 w-4" /> Riepilogo Collaboratori
            </CardTitle>
              ...
              {isLoading ? (
                <TableRow>
                  <TableCell colSpan={6} className="h-64 text-center">
                    ...
                    <p className="text-sm font-bold text-slate-400 mt-4">Calcolo in corso...</p>
                  </TableCell>
                </TableRow>
                ...
                <TableRow key={row.id} className="h-16 hover:bg-slate-50/50 transition-colors">
                  ...
                  <TableCell className="text-center">
                    <div className="flex flex-col items-center">
                      ...
                      <Badge variant="outline" className="text-[10px] h-4 bg-blue-50 border-blue-100 text-blue-600">PRESENZA</Badge>
                    </div>
                  </TableCell>
                  <TableCell className="text-center">
                    <div className="flex flex-col items-center">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <Card className="border-none shadow-sm bg-blue-50/50 border-l-4 border-l-[#227FD8]">
          <CardHeader className="p-5 pb-2">
            <CardTitle className="text-xs font-black uppercase text-blue-700 tracking-widest">Totale Ore Lavorate</CardTitle>
          </CardHeader>
        ...
        <Card className="border-none shadow-sm bg-amber-50/50 border-l-4 border-l-amber-500">
          <CardHeader className="p-5 pb-2">
            <CardTitle className="text-xs font-black uppercase text-amber-700 tracking-widest">Totale Ferie</CardTitle>
          </CardHeader>
          <CardContent className="p-5 pt-0">
            <p className="text-2xl font-black text-slate-900">
        ...
        <Card className="border-none shadow-sm bg-rose-50/50 border-l-4 border-l-rose-500">
          <CardHeader className="p-5 pb-2">
            <CardTitle className="text-xs font-black uppercase text-rose-700 tracking-widest">Totale Malattia</CardTitle>
          </CardHeader>
          <CardContent className="p-5 pt-0">
            <p className="text-2xl font-black text-slate-900">
